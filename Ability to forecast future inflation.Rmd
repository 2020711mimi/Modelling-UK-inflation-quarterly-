...
CPI.89_1_2021_10$log<-a
  GDPGRSub<-CPI.89_1_2021_10[CPI.89_1_2021_10$date_all>="2005-01-01" & CPI.89_1_2021_10$date_all<="2019-12-01",]
  # estimate the model
  #log lag selection 8
ar.ols(GDPGRSub$log,
       demean = F, 
...
#都是 选择 lag 8 AR(8)
library(tidyquant)
ts.df<- as.xts(GDPGRSub, date_col =date_all )
...
We also include the ability of current inflation to predict future inflation as a baseline in  comparison.
...
#CPI_baseline
res_cpi<-lapply(3:14, function(x) lm(CPI519[,x]~ CPI519$CPI))
...
kbl(together,caption = "Headline CPI Forecast Accuracy: Coefficients",col.names=c("$\\alpha$","$\\beta$","$\\alpha$","$\\beta$","$\\alpha$","$\\beta$","$\\alpha$","$\\beta$" ),escape = FALSE) %>%
  ...
  add_header_above(c(" " , "FE" = 2, "TR" = 2,"SP" = 2,"Baseline"=2)) %>%
...
kbl(together.rmse,caption = "Headline CPI Forecast Accuracy: Root Mean Squared Errors ",col.names=c("FE","TR","SP","Baseline" ),escape = FALSE) %>%
  kable_classic() %>%
  kable_styling(latex_options = c("striped", "hold_position","scale_down")) %>%
...
colnames(RMSE.ranking)<-c("FE", "TR", "SP","Baseline","FE", "TR", "SP","Baseline")
#<= 2MONTHS  FE>TR>SP
#=3M TR>FE>SP
#>3M TR>SP>FE
```
...
p<-ggplot(data=together.rmse, aes(x=idu))+
 geom_line(aes(y =RMSE_food, color="FE")) +
   ...
   geom_line(aes(y =RMSE_sticky, color="SP")) +
     geom_line(aes(y =RMSE_cpi, color="Baseline")) +
 ...
 geom_line(aes(y =c2, color="FE")) +
   geom_line(aes(y = c4, color="TR")) +
     geom_line(aes(y = c6, color="SP")) +
       geom_line(aes(y = c8, color="Baseline")) +
...
The $\beta$ of the Ex FE of the core inflation rate is between Trimmed mean and sticky CPI in this period.
...
From the root mean square error criteria, for months 1-2 the ranking is SP>TR>FE; month 3 SP>FE>TR; months 4-12 FE>SP>TR. All three similar.
