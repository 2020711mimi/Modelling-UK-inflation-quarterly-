...
together<-cbind(RMSE_food,cof_food_alpha,cof_food_beta,RMSE_trimmed,cof_trimmed_alpha,cof_trimmed_beta,RMSE_sticky,cof_sticky_alpha,cof_sticky_beta)
rownames(together)<-paste(1:12,"months ahead")
colnames(together)<-c("RMSE","$\\alpha$","$\\beta$","RMSE","$\\alpha$","$\\beta$","RMSE","$\\alpha$","$\\beta$" )

kbl(together,caption = "Headline CPI Forecast Accuracy: Root Mean Squared Errors and Beta",col.names=c("RMSE","$\\alpha$","$\\beta$","RMSE","$\\alpha$","$\\beta$","RMSE","$\\alpha$","$\\beta$" ),escape = FALSE) %>%
  kable_classic() %>%
  kable_styling(latex_options = c("striped", "hold_position","scale_down")) %>%
  add_header_above(c(" " , "Excluding Food and Energy" = 3, "Trimmed Mean" = 3,"Sticky Price CPI" = 3)) %>%
  add_header_above(c(" ", "Core Inflation " = 9))
...
colnames(together)<-paste0("c",1:9)
#ggplot
...
p<-ggplot(data=together, aes(x=idu))+
 geom_line(aes(y =c1, color="Ex F&E")) +
   geom_line(aes(y = c4, color="Trimmed Mean ")) +
   geom_line(aes(y =c7, color="Sticky CPI")) +
...
q<-ggplot(data=together, aes(x=idu))+
 geom_line(aes(y =c3, color="Ex F&E")) +
   geom_line(aes(y = c6, color="Trimmed Mean ")) +
     geom_line(aes(y = c9, color="Sticky CPI ")) +
...
Figure \@ref(fig:overall) shows that the predictive power of the core inflation of sticky CPI is the worst. The coefficient estimates of the Cogley regression deviate the furthest from - 1 and the predicted root mean square error (RMSE) was the largest over all forecast periods.
...
The $\beta$ of the Ex F&E of the core inflation rate is between Trimmed mean and sticky CPI in this period.
So from $\beta$ estimates, Trimmed Mean has the best prediction effect of core inflation rate when forecast horizon less 10 months.
The excluding food and energy core inflation rate is predicted to work best after 10 months periods. 
From the root mean square error criteria, the RMSE of sticky CPI is the largest, excluding food and energy and Trimmed mean are close but the former is smaller.
