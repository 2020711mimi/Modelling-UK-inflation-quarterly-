...
df_cpi <-
  rio::import("cpi ppi consumption unemployment/div.cpi97.csv")
df_ppi <-
  rio::import("cpi ppi consumption unemployment/ppi %97.csv")
df_unemploy <-
  rio::import("cpi ppi consumption unemployment/unemployment 97-19%.csv")
df_consumption <-
  rio::import("cpi ppi consumption unemployment/consumption monthly data change %.csv")
df_wage <-
  import("output, unemployment, PPI and hourly wages/wage %.csv")

df_consumption <-
  df_consumption[1:which(df_consumption$date == "2019-12-01"),]
#CPI lag create
for (i in 1:12) {
...
Consumption <-
  df_consumption[which(df_consumption$date == "2000-02-01"):nrow(df_consumption),]
...
stargazer(
  ...
  flip = FALSE,
  se = NULL,
  out = "output, unemployment, PPI and hourly wages/just consumption P.html",
  type = "text"
)
...
modelsummary(
  ...
  fmt = "%.2f",
  coef_omit = "Intercept",
  output = "output, unemployment, PPI and hourly wages/just consumption.html"
)
...
modelsummary(
  ...
  fmt = "%.2f",
  coef_omit = "Intercept",
  output = "output, unemployment, PPI and hourly wages/consumption AIC.html"
)
...
stargazer(
  ...
  flip = FALSE,
  se = NULL,
  out = "output, unemployment, PPI and hourly wages/consumption AIC P.html",
  type = "text"
)
