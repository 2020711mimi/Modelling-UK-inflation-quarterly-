...
for (i in 1:12) {
  ...
  #print(length(which(a[[i]] < 0.05))*weight[[2]])
...
write.table(y,"0.05.cpi.csv",sep=",")
y<-
percent(
  mapply(function(x1, y1,a,b1,c,d,e,f,h,i,j,k) x1[[1]]+y1[[1]]+a[[1]]+b1[[1]]+c[[1]]+d[[1]]+e[[1]]+f[[1]]+
           h[[1]]+i[[1]]+j[[1]]+k[[1]],
...
setwd("C:\\Users\\PC\\Desktop\\output\\7.18 reference output")
write.table(y,"0.01.cpi.csv",sep=",")
...
setwd("C:\\Users\\PC\\Desktop\\output\\7.18 reference output")
write.table(y,"0.05.lag.csv",sep=",")
y<-
percent(
  mapply(function(x1, y1,a,b1,c,d,e,f,h,i,j,k) x1[[1]]+y1[[1]]+a[[1]]+b1[[1]]+c[[1]]+d[[1]]+e[[1]]+f[[1]]+
           h[[1]]+i[[1]]+j[[1]]+k[[1]],
...
setwd("C:\\Users\\PC\\Desktop\\output\\7.18 reference output")
write.table(y,"0.01.lag.csv",sep=",")
...
# tidy for 0.05--------------------------------------------------------------------
slr.0.05 <- list() # regression 到122
for (i in 2:122) {
  slr.0.05[[i]] <- model.select(regression[[i]], keep = keep.dummies, sig = 0.05, verbose = F) # 122个?没有1和86
}
...
# for 0.01 ----------------------------------------------------------------
slr.0.01 <- list() # regression 到122
for (i in 2:122) {
  slr.0.01[[i]] <- model.select(regression[[i]], keep = keep.dummies, sig = 0.01, verbose = F) # 122个
}
# 替换变量名称
for (i in 2:122) {
...
#lag9 个数是34得都不对，（包含cpi——lag9） 19个才是对得，
for (i in 1:12) {
  #print(length(grep(x = a$term, pattern =paste0("\\blag",i,"\\b"))))
  print(percent(length(grep(x = a$term,  pattern =paste0("\\blag",i,"\\b")))/121))
}
