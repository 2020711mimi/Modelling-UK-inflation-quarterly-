...
q<-ggplot(data=u_i, aes(x=time))+
 geom_line(aes(y =unemployment, color="Unmployment rate (Growth rate)")) +
...
```{r CPI and 12,message=FALSE, include=FALSE, results='asis'}
library(stargazer)
#division 位置
div.poi<-vector()
...
## Regression output (grwoth rate)

The results are in (Table \@ref(fig:bar11)). As we can see, The unemployment change rate is not significant for the 12 divisions and aggregate CPI.



![(#fig:bar11)Consumption-Unemployment growth rate output](Regression-analysis-between-inflation-unemployment-and-real-GDP.png)


## The simplified result (grwoth rate)

Stepwise regression results (Table \@ref(fig:bar10)) confirm the previous conclusion that unemployment rate in the same period has no significant effect on CPI, no matter division or aggregate CPI level.

![(#fig:bar10)Consumption-Unemployment stepwise output](1 lag unemployment_simply.png)


```{r 12 lags simplified, include=FALSE}
...
slr.total[[2]]$coefficients

#替换名称
search_for_these <- c("lag1[, x]","lag2[, x]","lag3[, x]","lag4[, x]","lag5[, x]","lag6[, x]","lag7[, x]","lag8[, x]","lag9[, x]","lag10[, x]","lag11[, x]","lag12[, x]",
                      "growth2","growth3","growth4","growth5","growth6","growth7","growth8","growth9","growth10","growth11","growth12","growth13",
 "unemployment2"         )
 replace_with_these <- c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12",
                         "growth lag1","growth lag2","growth lag3","growth lag4","growth lag5","growth lag6","growth lag7","growth lag8",
                                        "growth lag9","growth lag10","growth lag11","growth lag12",
...
for (i in 1:length(slr.total)) {
 ...
 cpi.slr<- model.select(regression[[1]], keep = keep.dummies, sig = 0.05, verbose = F)
 found<-list()
  found[[1]] <- match(names(cpi.slr$coefficients), search_for_these, nomatch = 0)
  names(cpi.slr$coefficients)[names(cpi.slr$coefficients) %in% search_for_these] <- replace_with_these[found[[1]]]
```
...
vars.order<-c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12","CPI_lag1","CPI_lag2","CPI_lag3","CPI_lag4","CPI_lag5","CPI_lag6","CPI_lag7",
                                             "CPI_lag8","CPI_lag9",
                                             "CPI_lag10","CPI_lag11","CPI_lag12","growth lag1","growth lag2","growth lag3","growth lag4","growth lag5","growth lag6","growth lag7","growth lag8",
                                        "growth lag9","growth lag10","growth lag11","growth lag12","unemployment lag 1","unemployment lag 2","unemployment lag 3","unemployment lag 4","unemployment lag 5","unemployment lag 6","unemployment lag 7","unemployment lag 8","unemployment lag 9","unemployment lag 10","unemployment lag 11","unemployment lag 12")

abr.div.name<-c("01FB",	"02AT",	"03CF",	"04HW",	"05FH",	"06HL",	"07TR",	"08CM",	"09RC",	"10ED",	"11RH",	"12MS")
stargazer(cpi.slr,slr.total,
          star.cutoffs = c(0.05, 0.01),
...
## Some more lags of unemployment (Growth rate)

### Model and data

Since unemployment is not significant for division and aggregate CPI, we  try 12 more lags of unemployment. I'm  using the  unemployment growth rate  instead of level .
...
\end{aligned}
$$
```{r 12 lags growth ,include=FALSE}
regression <- lapply(1:122, function(x) lm(COPY[,x] ~ lag1[,x]+lag2[,x]+lag3[,x]+lag4[,x]+lag5[,x]+lag6[,x]+lag7[,x]+
                                             lag8[,x]+lag9[,x]+lag10[,x]+lag11[,x]+lag12[,x]+CPI_lag1+CPI_lag2+CPI_lag3+CPI_lag4+CPI_lag5+CPI_lag6+CPI_lag7+
                                             CPI_lag8+CPI_lag9+
                                             CPI_lag10+CPI_lag11+CPI_lag12+ time_Aug + 
                                             time_Dec+ time_Apr+ time_Jan+time_Jul+time_Jun +time_Mar+
                                             time_May+time_Nov+time_Oct+time_Sep+VAT1+VAT2+VAT3+Recession+Trend+growth2+growth3+growth4+growth5+growth6+growth7+growth8+growth9+growth10+growth11+growth12+growth13+unemployment_growth2+unemployment_growth3+unemployment_growth4+unemployment_growth5+unemployment_growth6+unemployment_growth7+unemployment_growth8+unemployment_growth9+unemployment_growth10+unemployment_growth11+unemployment_growth12+unemployment_growth13)
)
...
for (i in 1:122) {
  names(regression[[i]]$coefficients)<-c('Constant','lag1','lag2',"lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12",
                                         ...
                                         "time_May","time_Nov","time_Oct","time_Sep","VAT1","VAT2","VAT3","Recession","Trend",
                                         "growth lag1","growth lag2","growth lag3","growth lag4","growth lag5","growth lag6","growth lag7","growth lag8",
                                        "growth lag9","growth lag10","growth lag11","growth lag12","unemployment_growth lag 1","unemployment_growth lag 2","unemployment_growth lag 3","unemployment_growth lag 4","unemployment_growth lag 5","unemployment_growth lag 6","unemployment_growth lag 7","unemployment_growth lag 8","unemployment_growth lag 9","unemployment_growth lag 10","unemployment_growth lag 11","unemployment_growth lag 12")
}
...
```{r 12 lags  output,eval=FALSE, include=FALSE}
div.poi<-vector()
...
abr.div.name<-c("01FB",	"02AT",	"03CF",	"04HW",	"05FH",	"06HL",	"07TR",	"08CM",	"09RC",	"10ED",	"11RH",	"12MS")
stargazer(regression[[1]],div.reg,
          star.cutoffs = c(0.05, 0.01),
          dep.var.labels.include = FALSE,
          report = "vc*",
          align=TRUE,           
...
### Regression output (growth)

The results are in Table \@ref(fig:bar666). 


![(#fig:bar666)Consumption-12 lag Unemployment growth rate output](12 lag unemployment growth.png)



\newpage

### The simplified result (growth)

The results are in Table \@ref(fig:bar44).  Only 2 lagged variables of unemployment growth rate have significant influence on CPI aggregate, and the remaining 12 sectors have no influence at all.



![(#fig:bar44)Consumption-12 lag Unemployment growth rate simplified output](12 lag unemployment growth simply.png)

```{r 12  simplified, include=FALSE}
...
slr.total[[11]]$coefficients

#替换名称
search_for_these <- c("lag1[, x]","lag2[, x]","lag3[, x]","lag4[, x]","lag5[, x]","lag6[, x]","lag7[, x]","lag8[, x]","lag9[, x]","lag10[, x]","lag11[, x]","lag12[, x]",
                      "growth2","growth3","growth4","growth5","growth6","growth7","growth8","growth9","growth10","growth11","growth12","growth13",
 "unemployment2","unemployment3","unemployment4","unemployment5","unemployment6","unemployment7","unemployment8","unemployment9","unemployment10","unemployment11","unemployment12","unemployment13"         )
 replace_with_these <- c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12",
                         "growth lag1","growth lag2","growth lag3","growth lag4","growth lag5","growth lag6","growth lag7","growth lag8",
                                        "growth lag9","growth lag10","growth lag11","growth lag12",
                         "unemployment lag 1","unemployment lag 2","unemployment lag 3","unemployment lag 4","unemployment lag 5","unemployment lag 6","unemployment lag 7","unemployment lag 8","unemployment lag 9","unemployment lag 10","unemployment lag 11","unemployment lag 12"
                         )
 found<-list()
for (i in 1:length(slr.total)) {
  found[[i]] <- match(names(slr.total[[i]]$coefficients), search_for_these, nomatch = 0)
...
Now we repeat all the calculations using the level of the unemployment rate instead of growth.

```{r Unemployment level,include=FALSE}
u_rate<-read_excel("unemployment rate 97.3-19.11.xlsx")
Unemployment<-u_rate$`Unemployment rate`
```

The values of unemployment and inflation are entered into figure \@ref(fig:acde), which shows their development in the period from 01.03.1997 to 01.11.2019.

```{r acde, echo=FALSE, fig.cap="Unemployment and inflation", fig.topcaption=TRUE, message=FALSE, warning=FALSE,results='asis'}
u_i<-newdata[,1]
u_i$time<-newdata$time1
u_i$unemployment<-Unemployment
colnames(u_i)[1]<-c("Inflation")
...
#
q<-ggplot(data=u_i, aes(x=time))+
 geom_line(aes(y =unemployment, color="Unmployment rate (Level)")) +
   geom_line(aes(y =Inflation, color="Inflation(Monthly)")) +
labs(x="Time",y= "percent change")
q
```
...
```{r 12 lags ,include=FALSE}
regression <- lapply(1:122, function(x) lm(COPY[,x] ~ lag1[,x]+lag2[,x]+lag3[,x]+lag4[,x]+lag5[,x]+lag6[,x]+lag7[,x]+
                                             lag8[,x]+lag9[,x]+lag10[,x]+lag11[,x]+lag12[,x]+CPI_lag1+CPI_lag2+CPI_lag3+CPI_lag4+CPI_lag5+CPI_lag6+CPI_lag7+
                                             CPI_lag8+CPI_lag9+
                                             CPI_lag10+CPI_lag11+CPI_lag12+ time_Aug + 
                                             time_Dec+ time_Apr+ time_Jan+time_Jul+time_Jun +time_Mar+
                                             time_May+time_Nov+time_Oct+time_)
)
regression[[1]]$coefficients
for (i in 1:122) {
  names(regression[[i]]$coefficients)<-c('Constant','lag1','lag2',"lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12",
...
The results are in Table \@ref(fig:bar3). Some of unemployment lag variables are significant, especially for 06HL.

![(#fig:bar3)Consumption-12 lag Unemployment output](12 lags unemployment.png)

\newpage

## The simplified result

The results are in Table \@ref(fig:bar4).  The unemployment lag variables for 02AT, 03CF, 06HL, 09RC, 11RH and 12MS  are significant. It is worth noting that except for period lag of 1 and 2, which is not significant to 06HL, the other unemployment time lag variables are significant.

The most interesting things is that 2 period lag unemployment is insignificant for all divisions and aggregate CPI.

![(#fig:bar4)Consumption-12 lag Unemployment simplified output](12lag unemployment_ simplify.png)

```{r 12  simplified3, include=FALSE}
