...
df.lagleft$term[which(df.lagleft$p.value>0.05)]#大于0.05的全是intercept这是不是就说明所有都在0。05显著。
...
wide<-spread(df.lagleft, key = term, value =p.value)#我愿称之为奇迹！列是coef 名字， 行是每个值,列下面的行是每个值的p-value
asd<-wide[,-c(2,3,5)]#去掉stand erro，statistic,intercept列
asd<-asd[,c(1,2,3,7:14,4:6,15,19:26,16:18)]#重新排序，cpi 1：12，lag 1:12
ttt<-aggregate(x=asd[,3:26], by=list(name=asd$qwe), min, na.rm = TRUE)#合并行，把一个部门的从多行变为一行。
length(which(ttt[,2:25]<0.05))#所有值都显著在0.05
###0.05
aaa<-ttt#0.05合并的过程太慢了， 另存为一个好折腾
is.na(aaa)<-sapply(aaa, is.infinite)#把所有inf变成0 
aaa[is.na(aaa)]<-0
aaa[-1] <- as.integer(aaa[-1] != 0)#奇迹再次发生，把非0的变成1
write.csv(aaa,file = "aaa.csv",row.names = FALSE)
### 0.01
bbb<-ttt#0.01
bbb[-1]<-as.integer(bbb[-1]<0.01)#小于0.01的变成1,发现只需要interger函数就够了。自动把inf变成0了。
write.csv(bbb,file = "bbb.csv")
