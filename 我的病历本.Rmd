...
```{r}
backward <- lapply(1:122, function(i) step(regression[[i]], direction='backward', scope=formula(regression[[i]]), trace=0)
)
backward<-list()#不是324的运行不了 有些部门数据缺失
colnames(COPY)[!complete.cases(t(COPY))][1]#显示有na的列
for (i in 1:length(colnames(COPY)[!complete.cases(t(COPY))])) {
    print(which(colnames(COPY)==colnames(COPY)[!complete.cases(t(COPY))][i]))
}

which(colnames(COPY)[!complete.cases(t(COPY))])
not_any_na <- function(x) all(!is.na(x))
COPY<-COPY %>% select(where(not_any_na))
which(co)
for (i in 1: length(regression)) {
    backward <- step(regression[[i]], direction='backward', scope=formula(regression[[i]]), trace=0)
}
...
names(slr.cpi$coefficients)[2:3]<-c("lag1","lag2")
