...
这两种做法都和之前是完全相等. 都只针对**列**操作,对表格内容无关.
在此只保留
...
| ![论文第一表](F:/R%20workspace/Modelling-UK-inflation-quarterly-/table1.jpg) |
|:----------------------------------------------------------------------------:|
|                              <b>论文第一表</b>                               |
...
01 Food and non-alcoholic beverages **2-15** 02 Alcoholic beverages,
tobacco and narcotics **16-21** 03 Clothing and footwear **22-27** 04
Housing, water, electricity, gas and other fuels **28:40** 05
Furnishings, household equipment and routine household maintenance
**41:53** 06 Health **54:61** 07 Transport **62:76** 08 Communication
**77:79** 09 Recreation and culture **\[80:101** 10 Education **102** 11
Restaurants and hotels **103：107** 12 Miscellaneous goods and services
**108：122**

------------------------------------------------------------------------

因为每个部门现在是间断的了,不再连续,
所以之前根据constant转化的办法不行了. ## 手动生成a1:a12.
...
**再更新85版本之后, 可以用85版本的sig.class 替代
regression,避免了之后的操作**
...
```{r}
 length( which(test[[1]]$term==paste0("CPI_lag",1)&test[[1]]$p.value<0.01))
  #/length(get(paste0("a",1))) *weight[[1]]
### 2个显著,第一个部门11个regresion
```
...
一行一个结果,每个表有12行,共4个表,**4×12=48**个结果. 首先生成存放结果的
24行×2列的dataframe
...
写入结果 sig.class 转换成只有4列的长数据类型, 方便使用
...
#尝试stargazer把85个部门全都输出到html以便查找具体哪个显著

```{r}
df.summary<-data.frame()
x <- do.call(rbind, slr.total)
for (i in 1:length(slr.total)) {
  df.summary[<-(summary(slr.total[[i]])$coefficients)#把summary 导出到df

}
summary.df<-data.frame(summary(slr.total[[1:10]])$coefficients)#把summary 导出到df
omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
                   "VAT1","VAT2","VAT3","Recession","Trend",'Constant')#想忽略的变量

which(omit %in% rownames(summary.df)==TRUE )
omit.poi<-which(rownames(summary.df) %in% omit)
df.lagleft<-summary.df[-omit.poi,]
...
```{r}
df.dream<-data.frame(matrix(nrow = 85,ncol = 24))
colnames(df.dream)<-c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12","CPI_lag1","CPI_lag2","CPI_lag3","CPI_lag4","CPI_lag5","CPI_lag6","CPI_lag7",
                                             "CPI_lag8","CPI_lag9",
                                             "CPI_lag10","CPI_lag11","CPI_lag12")
for (i in 1:length(slr.total)) {
  if (summary(slr.total[[1]])$coefficients[,4]<0.05) {df.dream[i,]
    
  }
  
}
names(summary(slr.total[[1]])$coefficients[,4])
...
```{r}
df.85<-lapply(slr.total, tidy)#把regression list 化为一个data frmae
df85<- do.call(rbind.data.frame, df.85)
omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
                   "VAT1","VAT2","VAT3","Recession","Trend",'Constant')#想忽略的变量
omit.poi<-which(df85$term %in% omit)
df.lagleft<-df85[-omit.poi,]
df.name<-c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12","CPI_lag1","CPI_lag2","CPI_lag3","CPI_lag4","CPI_lag5","CPI_lag6","CPI_lag7",
                                             "CPI_lag8","CPI_lag9",
                                             "CPI_lag10","CPI_lag11","CPI_lag12")
length(which(df.lagleft$term=="(Intercept)"))#有84个
which(df.lagleft$p.value<0.05)#所有值都显著
start.poi<-which(df.lagleft$term=="(Intercept)")
write.csv(df.lagleft,"85个慢慢数把.csv") 


df.lagleft$qwe<-NA#生成na列
df.lagleft$qwe[start.poi]<-c(1:59,61:85)#na列intercept行插入1:59,61:85
df.lagleft<-df.lagleft %>% fill(qwe)#填充na
wide<-spread(df.lagleft, key = term, value =p.value)#我愿称之为奇迹！列是coef 名字， 行是每个值
asd<-wide[,-c(2,3,5)]#去掉stand erro之类的列
asd<-asd[,c(1,2,3,7:14,4:6,15,19:26,16:18)]#重新排序，cpi 1：12，lag 1:12
```
...
vars.order<-c("lag1","lag2","lag3","lag4","lag5","lag6","lag7","lag8","lag9","lag10","lag11","lag12","CPI_lag1","CPI_lag2","CPI_lag3","CPI_lag4","CPI_lag5","CPI_lag6","CPI_lag7",
                                             "CPI_lag8","CPI_lag9",
                                             "CPI_lag10","CPI_lag11","CPI_lag12")
name85.df<-read_excel("cpi weight for 85 divisions.xlsx")
name85<-dplyr::pull(name85.df, 1)

stargazer(slr.total[1:8],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
...
stargazer(slr.total[9:16],
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[9:16]),
          summary = FALSE,
          out = "85.2.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[17:24],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[17:24]),
          summary = FALSE,
          out = "85.3.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[25:32],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
...
stargazer(slr.total[33:40],
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[33:40]),
          summary = FALSE,
          out = "85.5.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[41:48],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[41:48]),
          summary = FALSE,
          out = "85.5.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[49:56],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[49:56]),
          summary = FALSE,
          out = "85.6.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[57:64],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[57:59],name85[61:65]),
          summary = FALSE,
          out = "85.7.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[65:72],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[66:73]),
          summary = FALSE,
          out = "85.8.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[73:80],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
          ...
          digits = 2, single.row = TRUE,
          model.numbers = FALSE,
          column.labels = c(name85[74:81]),
          summary = FALSE,
          out = "85.9.html",
          flip = FALSE,
          se = NULL,type = "html"
)
stargazer(slr.total[81:84],
          star.char = c("*"),
          star.cutoffs = c(0.01),
          notes = c(" * p<0.01; "),
          omit = c("time_Aug","time_Dec", "time_Apr", "time_Jan","time_Jul","time_Jun" ,"time_Mar","time_May","time_Nov","time_Oct","time_Sep",
...
**60属于第9部门** **根据 intercept 开始的位置, 和trend结束位置 划分df**
...
**修正summary table 在代码体现最大的变化应该是在run完regression的时候,
再把出去division and groupd的class 汇总在一起 生成新的sig.class,
在对季度数据应用的时候应当也是如此**
...
写入结果 一个没有cpi的a dataframe,删除前41行
