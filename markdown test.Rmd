...
=======
<<<<<<< HEAD
>>>>>>> parent of 2887626 (回滚到这里)
=======
<<<<<<< HEAD
>>>>>>> parent of 4fc7690 (最新版)
...
old13<-c(2:13)
  ...
  assign(paste0("lag",j),newdata%>%
           select(matches(paste0(old13[j],"$"))))
  
}
n2<-select(newdata,ends_with("2"))
for (i in 1:12) {

  assign(paste("lag",i),select(newdata,ends_with(paste0(old13[i]))))
...
=======
=======
>>>>>>> parent of faeefb7 (LAG END WITH 1 INCLUDING LAG11)
>>>>>>> parent of 4fc7690 (最新版)
```{r}
# LAG ---------------------------------------------------------------------
thesecols<-c(paste0("lag",3:12))

lag4<-newdata[,grep("4:13",names(newdata))]
<<<<<<< HEAD
=======
<<<<<<< HEAD
>>>>>>> parent of faeefb7 (LAG END WITH 1 INCLUDING LAG11)
...
for (i in 2251:2261) {
  ...
  assign(monthname[j],as.numeric(unlist(newdata[, ..i]))  )
  }
}
```
...
for (i in 1:12) {
  ...
  assign(paste0("CPI_lag",i),as.numeric(unlist(newdata[,..j])))
  }
}
