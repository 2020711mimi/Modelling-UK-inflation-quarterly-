...
which(a$term=="Trend")[1:15]
```
```{r}
which(a$term=="Constant")[2:15]
```
```{r}
atest1<-a[(which(a$term=="Constant")[dend[1]]):(which(a$term=="Trend")[dend[2]-1]),]
identical(a1,atest1)
which(a$term=="Constant")[2]
which(a$term=="Trend")[15]
which(a$term=="Trend")[dend[2]-1]
```
...
length(which(a1$term=="lag1"&a1$p.value<0.01))
...
108：122
上一个的Trend 位置加1 : 最后一个trend的位置.
第一个constant的位置,到下一个Trend-1的位置.

```{r}
dend<-c(2,15,16,21,22,27,28,40,41,53,54,61,62,76,77,79,80,101,102,107,103,107,108,122)
dend<-c(2,16,22,28,41,54,62,77,80,102,103,108)
for (i in 1:length(dend)) {print(dend[i+1])
    
}

length(dend)
for (i in 1:length(dend)) {
  a[i]<-a[(which(a$term=="Constant")[dend[i]]):(which(a$term=="Trend")[dend[i+1]-1]),]
}
for(i in 1:12){
  print{
which(a$term=="Constant")[dend[i+1]]
  }
  }
```
