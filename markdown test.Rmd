...
```{r}
sum(a_cpi.0.05["CPI_lag1",])/121*100

```

```{r}
table1weight<-data.frame( matrix(nrow=24, ncol=2, )   )
colnames(table1weight)<-c(0.01,0.05)
rownames(table1weight)<-c(paste0("CPI_lag",1:12),paste0("lag",1:12))
  ...
  for (i in 1:12) {
    table1weight[i,1]<-percent(sum(a_cpi.0.01[paste0("CPI_lag",i),])      /121)
    table1weight[i+12,1]<-percent(sum(a_lag.0.01[paste0("lag",i),])/121)
    table1weight[i,2]<-percent(sum(a_cpi.0.05[paste0("CPI_lag",i),])/121)
    table1weight[i+12,2]<-percent(sum(a_lag.0.05[paste0("lag",i),])/121)
    
  }
kable(table1weight, caption = "Table 1")
writeDoc(doc, file = "kable.docx")
```
