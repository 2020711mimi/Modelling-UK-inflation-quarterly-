...
assign(
  get(paste0("a",j,"lag")),
         length(
    which((get(paste0("a",j)))$term==paste0("lag",i) &(get(paste0("a",j)))$p.value<0.01)
  )*weight[[j]])
  ...
  #a1to12lag1[1]:a1 lag1 且a1<0.01的*weight[1]
  #j[1]:a1 lag1且a1中小于0.01的*weight1
  #j[2]:a1 lag2且a1中小于0.01的*weight2
  #....j[12]:...
  #back to first
  #a1to12lag1[2]:a2 lag1 0.01*weight[2]
  #j[2]:a2 lag小于0.01的个数*weight[2]
...
```
### 选中a1:a12
f%02d %02d好像是指前面可以有两个0.
```{r}
features <- c(sprintf("a%01d", seq(1,12)))
colnames(urc_training_norm) <- features
alist<-list(get(paste0("a",1:12)))
```
